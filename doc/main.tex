...
As previously mentioned, there has been limited research focused on Persian medical language models, highlighting a significant gap in resources for the Persian-speaking medical community. Furthermore, existing works on Persian medical question-answering systems are entirely closed-source regarding their datasets, models, and codebases. This lack of public resources leaves the field largely underexplored, presenting researchers with an almost blank slate to build upon. On the other hand, all of these efforts have primarily concentrated on extractive solutions, which aim to retrieve relevant information from predefined sources, rather than employing generative approaches capable of producing context-aware responses. In contrast to all previous studies in the Persian language domain, our approach is generative, not extractive, leveraging the internal knowledge of a language model to produce answers by synthesizing context and understanding. Generative question-answering systems rely on the internal knowledge of a language model, enabling them to produce answers by synthesizing context and understanding, whereas extractive question-answering systems retrieve answers from expert-generated data. For a detailed comparison of these two approaches, please refer to Table \ref{tab:ext_gen_comparison}.
	...
	Perhaps the most notable effort in the Persian medical language models field is Sina-BERT \cite{b4}, which involved training a BERT model \cite{b11}
...
We fine-tuned the 8-billion-parameter checkpoint of the aya-expanse model on 60\% of our corpus, focusing on minimizing resource usage. To ensure an efficient fine-tuning process, we employed gradient checkpointing and a small batch size of 2,reducing memory requirements during training. Additionally,we used gradient accumulation steps of 16, effectively increasing the overall batch size to 32 and enabling stable training dynamics. To further reduce the memory usage of our fine-tuning process, we leveraged Low Rank Adaptation (LoRA) \cite{b28} to significantly reduce the number of trainable parameters. Specifically, we implemented a rank of 8, an alpha value of 16, and a dropout rate of 5\%, applying the LoRA weights to all trainable parameters of each transformer layer.To further optimize the process, we employed efficient tokenization and memory-aware training techniques. The tokenization process divided the input text into manageable token sequences, ensuring consistent input and label structures by truncating, padding, and handling overflowed tokens to maintain contextual integrity within a fixed context length. This streamlined preparation, coupled with the LoRA-based fine-tuning, was further enhanced by Flash Attention 2 \cite{b29}.By minimizing memory overhead, Flash Attention 2 allowed us to handle longer context lengths and larger batch sizes efficiently, enabling effective fine-tuning for the next-token prediction objective while balancing computational efficiency with model performance.
...
Currently, there are no models tailored for medical translation in the Persian language, which means that existing systems are ill-equipped to handle the complexities of medical terminology. As shown in Figure \ref{fig6}, these limitations have resulted in all pipeline alternatives achieving lower win rates against our model, Gaokerena-V. This performance gap highlights the inadequacies of current pipeline approaches in meeting the demands of medical translation, emphasizing the necessity for the development of dedicated medical translator models that can effectively address these specific challenges.
	...
	\end{figure}
        ...
        As an ablation study, we trained qwen2.5 using exactly the same methodology and training setup as employed for aya-expanse. This allowed us to directly compare the performance of the two baseline models under identical conditions,
       the results of this comparison are presented in Figure \ref{fig7} and detailed further in Table \ref{tab:gaokernaV_vs_qwen2.5trained}, where it is evident that the trained qwen-2.5 model achieved lower scores across key performance metrics in comparison with gaokerena-V, which utilized Aya-Expanse as its baseline model. These findings highlight the advantages of using aya-expanse as the backbone model and reinforce its effectiveness in producing higher-performing models.
	...
	\begin{table}[ht]
		...
		\begin{tabular}{|l|c|c|c|}  % Using vertical lines for a simple table
			\hline
			\textbf{} & \textbf{gaokerena-V} 
			& \textbf{Qwen2.5} & \textbf{Qwen2.5} \\ 
			& (ours)  & &\textbf{trained}  \\ \hline
			MMLU- &  &  &  \\ 
			anatomy(fa)  & \textbf{48.14} & 41.48 & 36.29   \\ \hline
			MMLU- &    &  &  \\
			medical-genetics(fa) & \textbf{53.0} & 52.0 & 50.0 \\ \hline
			MMLU- &  &    &  \\
			college-medicine(fa) & \textbf{43.93} & 43.35 & 42.77 \\ \hline
			MMLU- &    &  &  \\
			clinical-knowledge(fa)& \textbf{55.47} & 47.92 & 49.81 \\ \hline
			MMLU- &  &  &  \\
			professional-medicine(fa)& \textbf{47.05} & 43.01 & 42.27 \\ \hline
			MMLU- &  &  &  \\
			college-biology(fa)& 47.22 & 42.36 &  \textbf{0.50}\\ \hline
			MMLU(avg) & \textbf{49.31} & 45.17 & 45.35  \\ \hline
			IBMSEE Sept2023 & \textbf{38.69}  & 33.33 &  35.71 \\ \hline
			Number of&  &  &  \\
			parameters & 8b & 7.6b & 7.6b \\ \hline
			inference time & $\approx 10s$ & $\approx 15s$ & $\approx 15s$ \\  \hline
		\end{tabular}
		\label{tab:gaokernaV_vs_qwen2.5trained}
	\end{table}


	\begin{figure}[htbp]
		\centerline{\includegraphics[width=0.45\textwidth]{fig7.png}}
		\caption{Our model win rate against trained qwen2.5}
		\label{fig7}
	\end{figure}

	\section*{Future research}
	As mentioned in the baseline model section, we can merge our updated parameters into aya-vision instead of aya-expanse, This will allow us to incorporate medical images, such as MRIs and CT scans, as inputs to our model.
	However, it is important to note that our models capabilities with multi-modal queries are under-explored.
	Our future research plans include exploring and improving the model’s responses to queries accompanied by medical images. This will involve fine-tuning the model to better process and align textual and visual data, with the goal of expanding its utility in medical diagnostics and analysis.
